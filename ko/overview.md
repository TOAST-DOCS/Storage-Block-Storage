...
- 루트 블록 스토리지의 저장 공간이 부족할 때 블록 스토리지를 추가로 장착하여 인스턴스의 저장 공간을 늘일 수 있습니다.
...
먼저 빈 장치인 `vdb`에 파티션을 생성합니다. 다음와 같이 `fdisk` 유틸리티를 이용하여 블록 스토리지 전체를 파티션 하나로 만듭니다. 필요에 따라 블록 스토리지 하나를 여러 파티션으로 나눌 수도 있습니다.
```
# fdisk /dev/vdb
...
#### 블록 스토리지 마운트
파일 시스템까지 만든 블록 스토리지에는 마운트 과정을 거쳐야 접근할 수 있습니다. 간편하게 `mount` 명령어로 블록 스토리지를 마운트할 수도 있으나 인스턴스가 재부팅되면 마운트 해제(unmount)됩니다. 이 예제에서는 `/etc/fstab` 파일에 마운트할 블록 스토리지를 추가하여 인스턴스 부팅 과정에서 자동으로 마운트하는 방법을 설명합니다.
...
마지막으로 `/etc/fstab`의 내용을 반영해야 합니다. `mount -a` 명령어로 `/etc/fstab`에 등록된 모든 블록 스토리지를 마운트합니다.
```
# mount -a
```
