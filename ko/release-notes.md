<!-- pre-align:aligned sig=4853fbdb9c19 -->

<a id="storage-block-storage-release-notes"></a>
## Storage > Block Storage > 릴리스 노트 { #storage-block-storage-release-notes }

<a id="may-27-2026"></a>
## 2026. 05. 27. { #may-27-2026 }

<a id="feature-updates"></a>
### 기능 개선/변경 { #feature-updates }

* 블록 스토리지 연결 정보 표기 개선
    * 종료된 인스턴스에 연결된 블록 스토리지도 어느 인스턴스에 연결되어 있는지 확인할 수 있습니다.
    * 기존 **연결 정보** 열이 **연결 인스턴스** 열로 변경되었습니다.
* 블록 스토리지 및 스냅숏 목록 조회 API의 limit 파라미터 기본값(default)이 100개, 최댓값(max)이 1,000개로 조정됩니다.
* 블록 스토리지 **연결 관리** 버튼 분리
    * 기존 **연결 관리** 버튼이 **연결 추가**와 **연결 해제** 버튼으로 분리됩니다.

<a id="may-27-2025"></a>
## 2025. 05. 27. { #may-27-2025 }

<a id="added-features"></a>
### 기능 추가 { #added-features }

* 블록 스토리지 이동 기능이 추가됩니다.
* 블록 스토리지 연결 시 삭제 정책 설정 기능이 추가됩니다.

<a id="march-4-2025"></a>
## 2025. 03. 04. { #march-4-2025 }

<a id="march-4-2025-added-features"></a>
### 기능 추가 { #march-4-2025-added-features }

* 블록 스토리지당 최대 스냅숏 수를 제한하는 쿼터가 추가됩니다.

<a id="august-27-2024"></a>
## 2024. 08. 27. { #august-27-2024 }

<a id="august-27-2024-added-features"></a>
### 기능 추가 { #august-27-2024-added-features }

* 동일 리전을 대상으로 블록 스토리지를 복제할 수 있는 기능이 추가됩니다.
* 자신이 속한 다른 프로젝트 대상으로 블록 스토리지를 복제할 수 있는 기능이 추가됩니다.

<a id="may-28-2024"></a>
## 2024. 05. 28. { #may-28-2024 }

<a id="may-28-2024-added-features"></a>
### 기능 추가 { #may-28-2024-added-features }

* 암호화 블록 스토리지의 스냅숏 생성 기능이 추가됩니다.
* 암호화 블록 스토리지의 리전 간 복제 기능이 추가됩니다.
* 블록 스토리지 크기 변경 기능이 추가됩니다.

<a id="april-23-2024"></a>
## 2024. 04. 23. { #april-23-2024 }

<a id="april-23-2024-feature-updates"></a>
### 기능 개선/변경 { #april-23-2024-feature-updates }

* 한국(판교) 리전에서 u2 전용 스토리지 타입 이용이 종료됩니다.

<a id="august-29-2023"></a>
## 2023. 08. 29. { #august-29-2023 }

<a id="august-29-2023-feature-updates"></a>
### 기능 개선/변경 { #august-29-2023-feature-updates }

* 용어 변경
    * 볼륨, 디스크 등의 용어를 **블록 스토리지**로 변경했습니다.

<a id="may-30-2023"></a>
## 2023. 05. 30. { #may-30-2023 }

<a id="may-30-2023-added-features"></a>
### 기능 추가 { #may-30-2023-added-features }

* 암호화 블록 스토리지 타입이 추가됩니다.

<a id="march-28-2023"></a>
## 2023. 03. 28. { #march-28-2023 }

<a id="march-28-2023-feature-updates"></a>
### 기능 개선/변경 { #march-28-2023-feature-updates }

* API 엔드포인트가 변경됩니다.

<a id="september-27-2022"></a>
## 2022. 09. 27. { #september-27-2022 }

<a id="september-27-2022-feature-updates"></a>
### 기능 개선/변경 { #september-27-2022-feature-updates }

* **스냅숏에서 블록 스토리지 생성** 기능 수행 시 스냅숏 무결성 확인 절차가 추가됩니다.

<a id="march-29-2022"></a>
## 2022. 03. 29. { #march-29-2022 }

<a id="march-29-2022-added-features"></a>
### 기능 추가 { #march-29-2022-added-features }

* 리전 간 복제 기능이 추가됩니다.

<a id="may-26-2020"></a>
## 2020. 05. 26. { #may-26-2020 }

<a id="may-26-2020-added-features"></a>
### 기능 추가 { #may-26-2020-added-features }

* Public API v2 출시
    * Public API v2는 Openstack API와 호환됩니다.

<a id="march-24-2020"></a>
## 2020. 03. 24. { #march-24-2020 }

<a id="march-24-2020-feature-updates"></a>
### 기능 개선/변경 { #march-24-2020-feature-updates }

* 생성할 수 있는 블록 스토리지의 최대 크기가 1TB에서 2TB로 증가합니다.

<a id="november-26-2019"></a>
## 2019. 11. 26. { #november-26-2019 }

<a id="november-26-2019-feature-updates"></a>
### 기능 개선/변경 { #november-26-2019-feature-updates }

* 기본 디스크가 블록 스토리지 목록에 노출됩니다.

<a id="january-29-2019"></a>
## 2019. 01. 29. { #january-29-2019 }

<a id="january-29-2019-added-features"></a>
### 기능 개선/변경 { #january-29-2019-added-features }

* 블록 스토리지 생성 시 원본으로 스냅숏을 선택할 수 없도록 변경됩니다.
* 스냅숏으로 블록 스토리지 생성 시 가용성 영역과 블록 타입을 선택할 수 있도록 변경됩니다.

<a id="september-21-2017"></a>
## 2017. 09. 21. { #september-21-2017 }

<a id="september-21-2017-added-features"></a>
### 기능 추가 { #september-21-2017-added-features }

* Public API 추가
    * Object Storage에 이어 Block Storage를 API로 사용할 수 있습니다.
    * 현재 제한적인 기능만 이용할 수 있으며, 추후 API가 추가되어 기능이 확장될 예정입니다.
    * [블록 스토리지 API 가이드](/Storage/Block%20Storage/ko/public-api/)를 참고하세요.

<a id="july-20-2017"></a>
## 2017. 07. 20. { #july-20-2017 }

<a id="bug-fixes"></a>
### 버그 수정 { #bug-fixes }

* 블록 스토리지 생성 시 간헐적으로 생성이 완료되지 않던 버그가 수정되었습니다.

<a id="january-19-2017"></a>
## 2017. 01. 19. { #january-19-2017 }

<a id="january-19-2017-feature-updates"></a>
### 기능 개선/변경 { #january-19-2017-feature-updates }

* 블록 스토리지 연결 관리 기능에 동일 가용성 영역의 인스턴스에만 연결이 가능함을 명시했습니다.

<a id="january-19-2017-bug-fixes"></a>
### 버그 수정 { #january-19-2017-bug-fixes }

* 스냅숏으로 블록 스토리지 생성 시 간헐적으로 실패하는 문제가 수정되었습니다.