...
* 블록 스토리지 연결 정보 표기 개선
    * 종료된 인스턴스에 연결된 블록 스토리지에 대해서도 어느 인스턴스에 연결되어 있는지 확인 가능
    * 기존 "연결 정보" 컬럼을 "연결 인스턴스" 컬럼으로 표기 정보 변경
* 블록 스토리지 및 스냅숏 목록 조회 API의 limit 파라미터 기본값(default) 100개, 최댓값(max) 1000개로 조정
* 블록 스토리지 연결 관리 버튼 분리
    * 기존 "연결 관리" 버튼을 "연결 추가"와 "연결 해제" 버튼으로 분리
...
* Public API 추가
    ...
    * [블록 스토리지 API 가이드](/Storage/Block%20Storage/ko/public-api/)를 참고하시기 바랍니다.
