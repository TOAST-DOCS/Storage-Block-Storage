...
ブロックストレージはインスタンス基本ディスク以外に追加で接続して使用できる仮想ディスクです。
...
- 基本ディスクの記憶領域が不足した時、ブロックストレージを追加で装着してインスタンスの記憶領域を増やすことができます。
- インスタンスを削除する前に、インスタンスの基本ディスクにあるデータを永久保存するために、ブロックストレージを装着してデータをコピーできます。
...
> [参考] Windowsのパーティション形式は2種類です。
> * MBR：以前から使用されているパーティション形式で、 2TB以下のディスクで使用します。
> * GPT： 2TB以上のディスクで使用する新しいパーティション形式です。
...
ブロックストレージは、作成した瞬間から使用した記憶領域分の料金がかかり、空のブロックのストレージやスナップショットで最終作成されるブロックストレージのサイズ分、料金がかかります。
